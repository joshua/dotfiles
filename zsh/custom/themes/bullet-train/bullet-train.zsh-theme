...
# Go
# - Only shows the go version if within the GOPATH
prompt_go() {
...
prompt_aws() {
  ...
  local profile="NO DEFAULT"
  [[ ${#AWS_DEFAULT_PROFILE} -gt 0 ]] && profile="$AWS_DEFAULT_PROFILE"
  prompt_segment $BULLETTRAIN_AWS_BG $BULLETTRAIN_AWS_FG $BULLETTRAIN_AWS_PREFIX"$profile"
}
